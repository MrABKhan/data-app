function Gallery() {
    ...
    this.selectVisual = function (visId) {
        ...
        //Create options area at side bar
        if (!document.contains(document.getElementById("visual-options"))) {

            var options = createDiv("Options").id("visual-options");
            options.parent('side-bar');
            options.style('padding', '18px');
            options.style('font-weight', 'bold');
            options.style('background-color', '#DDDDDD');

        }

        //Add a download button only if it does not already exist
        if (!document.contains(document.getElementById("save-button"))) {
            //Add save button when a visualisation is selected
            var myButton = createButton("Save As Image").id("save-button")
            myButton.style('margin', '5px');
            var buttonArea = select('#buttons-area');
            buttonArea.child(myButton);

            // Create new save visual Object
            saveVisual = new saveVisual();
        }
